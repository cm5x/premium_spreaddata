...
print("df.shape",merged_data.shape)
print("df.info()",merged_data.info())
print("df.columns", merged_data.columns)
print("df.describe()", merged_data.describe())
print("df.head()", merged_data.head())
    ...
    for row in ws.iter_rows(min_row=2, min_col=col_idx+1, max_col=col_idx+1, max_row=ws.max_row):
        for cell in row:
            if isinstance(cell.value, (int, float)):  # Ensure it's a numeric value
                ...
                elif 1.25 * arb_threshold <= cell.value <  20 * arb_threshold:  # Dark Green
